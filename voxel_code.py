...
Links of access to "UP000005640_9606_HUMAN" folder that used in this project:
https://drive.google.com/drive/folders/1Vj1vudfONxoPWt9yE-gyfEZOtX5S-4rc?usp=sharing
"""
...
def input_file(file):
    ...
    def inside_voxel_test(points, centre_points):
    ...
    def check_for_all_elements(points, centre_coords):
        _res = np.array([inside_voxel_test(points, i) for i in centre_coords])
...
def main():
    ...
    for filename in filename_lst:  # Save the first 50 .pdb files inside_test result of each residue
        f_name = os.path.splitext(filename)[0] + "_coor.npy"
        new_filename_lst_coor.append(f_name)

    for filename in filename_lst:  # Save the first 50 .pdb files confident result of each residue
        f_name = os.path.splitext(filename)[0] + "_confi.npy"
        new_filename_lst_confi.append(f_name)

    for filename in filename_lst:  # Test for the first 50 .pdb file
